...
def auth_headers(client):
    resp = client.post("/prototype/auth/login", json={"email": TEST_EMAIL, "password": TEST_PASSWORD})
    assert resp.status_code == 200, f"Login failed: {resp.text}"
    token = resp.json()["access_token"]
    return {"Authorization": f"Bearer {token}"}


def post(client, path, json, headers=None):
    r = client.post(f"/prototype{path}", json=json, headers=headers)
    assert r.status_code == 200, r.text
    return r.json()


def get(client, path, headers=None):
    r = client.get(f"/prototype{path}", headers=headers)
    assert r.status_code == 200, r.text
    return r.json()
...
# ============================================================
def test_checkout_with_copay_collected(client, auth_headers):
    """
    ...
    patient = post(client, "/patients", {"first_name": "John", "last_name": "Doe", "date_of_birth": "1990-01-01", "phone": "555-1111"}, headers=auth_headers)
...
# ============================================================
def test_active_visits_excludes_checked_out(client, auth_headers):
    """
    ...
    """
    room = post(client, "/admin/rooms", {"name": "R2", "code": "R2", "room_type": "treatment"}, headers=auth_headers)
    staff = post(client, "/admin/staff", {"name": "Dr. Smith", "role": "doctor"}, headers=auth_headers)
    patient = post(client, "/patients", {"first_name": "Jane", "last_name": "Smith", "date_of_birth": "1990-01-01", "phone": "555-2222"}, headers=auth_headers)
    ...
    p2 = post(client, "/patients", {"first_name": "Bob", "last_name": "Lee", "date_of_birth": "1990-01-01", "phone": "555-3333"}, headers=auth_headers)
    ...
    active1 = get(client, "/projections/active-visits", headers=auth_headers)
...
# ============================================================
def test_patient_visit_history_shows_all_visits(client, auth_headers):
    """
    ...
    """
    room = post(client, "/admin/rooms", {"name": "R3", "code": "R3", "room_type": "treatment"}, headers=auth_headers)
    staff = post(client, "/admin/staff", {"name": "PT Mary", "role": "therapist"}, headers=auth_headers)
    patient = post(client, "/patients", {"first_name": "Alice", "last_name": "Wong", "date_of_birth": "1990-01-01", "phone": "555-4444"}, headers=auth_headers)
    ...
    history = get(client, f"/patients/{patient['patient_id']}/visits", headers=auth_headers)
...
# ============================================================
def test_wd_verified_field_works(client, auth_headers):
    """
    TC-4: wd_verified field can be set True/False
    Expected: Field persists correctly and defaults to False
    """
    room = post(client, "/admin/rooms", {"name": "R4", "code": "R4", "room_type": "treatment"}, headers=auth_headers)
    staff = post(client, "/admin/staff", {"name": "Dr. Lee", "role": "doctor"}, headers=auth_headers)
    patient = post(client, "/patients", {"first_name": "Test", "last_name": "WD", "date_of_birth": "1990-01-01", "phone": "555-5555"}, headers=auth_headers)
...
# ============================================================
def test_patient_signed_field_works(client, auth_headers):
    """
    TC-5: patient_signed field can be set True/False
    Expected: Field persists correctly and defaults to False
    """
    room = post(client, "/admin/rooms", {"name": "R5", "code": "R5", "room_type": "treatment"}, headers=auth_headers)
    staff = post(client, "/admin/staff", {"name": "Front Desk", "role": "admin"}, headers=auth_headers)
    patient = post(client, "/patients", {"first_name": "Signed", "last_name": "Test", "date_of_birth": "1990-01-01", "phone": "555-6666"}, headers=auth_headers)
...
# ============================================================
def test_pdf_sign_sheet_generation(client, auth_headers):
    """
    TC-6: PDF sign-sheet endpoint generates valid PDF bytes
    Expected: Returns application/pdf content with correct filename
    """
    patient = post(client, "/patients", {"first_name": "PDF", "last_name": "Test", "date_of_birth": "1990-01-01", "phone": "555-7777"}, headers=auth_headers)
    room = post(client, "/admin/rooms", {"name": "R6", "code": "R6", "room_type": "treatment"}, headers=auth_headers)
    staff = post(client, "/admin/staff", {"name": "OT Mike", "role": "therapist"}, headers=auth_headers)
    ...
    # Get PDF
    pdf_resp = client.get(f"/prototype/patients/{patient['patient_id']}/sign-sheet.pdf", headers=auth_headers)
...
# ============================================================
def test_daily_summary_copay_total(client, auth_headers):
    """
    ...
    room = post(client, "/admin/rooms", {"name": "R7", "code": "R7", "room_type": "treatment"}, headers=auth_headers)
    staff = post(client, "/admin/staff", {"name": "Multiple", "role": "therapist"}, headers=auth_headers)

    # Create 3 patients with different copays
    copays = [10.0, 20.0, 35.50]
    for i, copay in enumerate(copays):
        p = post(client, "/patients", {"first_name": f"Patient{i}", "last_name": "Copay", "date_of_birth": "1990-01-01", "phone": f"555-800{i}"}, headers=auth_headers)
        v = post(client, "/portal/checkin", {
            "patient_ref": "walk-in",
    ...
    summary = get(client, "/projections/daily-summary", headers=auth_headers)
...
# ============================================================
def test_negative_copay_rejected(client, auth_headers):
    """
    TC-E1: Negative copay values should be rejected or handled
    Expected: System either rejects or applies validation
    """
    room = post(client, "/admin/rooms", {"name": "R8", "code": "R8", "room_type": "treatment"}, headers=auth_headers)
    staff = post(client, "/admin/staff", {"name": "Edge", "role": "therapist"}, headers=auth_headers)
    patient = post(client, "/patients", {"first_name": "Negative", "last_name": "Test", "date_of_birth": "1990-01-01", "phone": "555-9999"}, headers=auth_headers)
...
# ============================================================
def test_zero_copay_allowed(client, auth_headers):
    """
    TC-E2: Zero copay should be valid (e.g., insurance-only visits)
    Expected: copay_collected=0.0 is valid
    """
    room = post(client, "/admin/rooms", {"name": "R9", "code": "R9", "room_type": "treatment"}, headers=auth_headers)
    staff = post(client, "/admin/staff", {"name": "Zero", "role": "therapist"}, headers=auth_headers)
    patient = post(client, "/patients", {"first_name": "Zero", "last_name": "Copay", "date_of_birth": "1990-01-01", "phone": "555-0000"}, headers=auth_headers)
...
# ============================================================
def test_event_log_no_phi_in_patient_name(client, auth_headers):
    """
    ...
    """
    room = post(client, "/admin/rooms", {"name": "R10", "code": "R10", "room_type": "treatment"}, headers=auth_headers)
    staff = post(client, "/admin/staff", {"name": "PHI Test", "role": "therapist"}, headers=auth_headers)
    patient = post(client, "/patients", {"first_name": "SECRET", "last_name": "PHI", "date_of_birth": "1990-01-01", "phone": "555-PHI1"}, headers=auth_headers)
    ...
    # Get event log
    events = get(client, "/events", headers=auth_headers)
