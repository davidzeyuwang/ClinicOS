...
async def test_prd005_multiple_treatments_workflow():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        # Setup
        await client.post("/prototype/test/reset")
        ...
        # 5. List treatments for visit
        list_resp = await client.get(f"/prototype/visits/{visit_id}/treatments")
        ...
        # 6. Query treatment records
        records_resp = await client.get("/prototype/treatment-records")
        ...
        # Filter by modality
        filtered = await client.get("/prototype/treatment-records?modality=E-stim")
        assert filtered.status_code == 200
        assert len(filtered.json()["treatments"]) == 1
        assert filtered.json()["treatments"][0]["modality"] == "E-stim"
        
        # Filter by therapist
        therapist_records = await client.get(f"/prototype/treatment-records?staff_id={therapist2_id}")
        ...
        # 7. Delete one treatment
        delete_resp = await client.delete(f"/prototype/visits/{visit_id}/treatments/{t2_id}/delete")
        assert delete_resp.status_code == 200
        assert delete_resp.json()["deleted"] is True
        
        # Verify deletion
        list_after_delete = await client.get(f"/prototype/visits/{visit_id}/treatments")
        ...
        # 9. Verify events logged
        events_resp = await client.get("/prototype/events")
...
async def test_treatment_records_date_filter():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        future = "2099-12-31"
        records_future = await client.get(f"/prototype/treatment-records?date_from={future}&date_to={future}")
